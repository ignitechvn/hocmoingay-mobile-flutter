...
class ExamReportDetailScreen extends ConsumerStatefulWidget {
  final String classroomId;
  final String classroomName;
  final String examId;
  final String examName;

  const ExamReportDetailScreen({
    super.key,
  ...
  @override
  ConsumerState<ExamReportDetailScreen> createState() =>
      _ExamReportDetailScreenState();
}

class _ExamReportDetailScreenState
    extends ConsumerState<ExamReportDetailScreen> {
  @override
  Widget build(BuildContext context) {
    final reportAsync = ref.watch(examReportProvider(widget.examId));

    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(
        title: Text(
          'Báo cáo ${widget.examName}',
        ...
        error:
            (error, stack) => Center(
              child: EmptyStateWidgets.error(
                message: 'Không thể tải báo cáo bài thi',
                onRetry: () {
                  ref.invalidate(examReportProvider(widget.examId));
                },
              ),
            ),
      ),
  ...
  Widget _buildContent(BuildContext context, ExamReportDto report) {
  ...
  Widget _buildHeader(ExamReportDto report) {
    return Card(
      elevation: 0,
  ...
  Widget _buildQuestionsList(ExamReportDto report) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
